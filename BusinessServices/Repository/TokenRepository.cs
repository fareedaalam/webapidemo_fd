...
namespace BusinessServices.Repository
{
    public class TokenRepository:ITokenInterface
    {
       ...
       // private readonly object ConfigurationManager;
        ...
        {
            var seconds = Convert.ToDouble(ConfigurationManager.AppSettings["AuthTokenExpiry"]);

            string token = Guid.NewGuid().ToString();
            DateTime issuedOn = DateTime.Now;           
            DateTime expiredOn = DateTime.Now.AddSeconds(seconds);
       ...
       // Method to validate token against expiry and existence in database.
        public bool ValidateToken(string tokenId)
        {
            ...
            {
                var seconds = Convert.ToDouble(ConfigurationManager.AppSettings["AuthTokenExpiry"]);
                token.ExpiresOn = token.ExpiresOn.Value.AddSeconds(seconds);               
